...
import { useGetCustomerQuery } from "@/lib/features/auth/authCustomer"; // Import your customer query hook

export default function CardSettings() {
  const unique_id = localStorage.getItem("unique_id");
  const role = localStorage.getItem("role"); // Assuming role is stored in localStorage

  // Fetch merchant and customer data
  const {
    data: merchant,
    error: merchantError,
    isLoading: isLoadingMerchant,
  } = useGetMerchantQuery(unique_id);
  const {
    data: customer,
    error: customerError,
